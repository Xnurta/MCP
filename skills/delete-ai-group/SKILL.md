...
description: >-
  ...
  group (use edit-ai-group), editing config (use edit-ai-group), or creating a group (use
  create-ai-group).
metadata:
  version: 1.0.0
---
...
1. **Resolve the group to an ID.** Users refer to groups by name - look it up with
   ...
   - **Also capture the group's full campaign list.** Query `entity='campaign'` filtered
     by this `aiGroupId` and **save the campaign IDs** - you need the actual IDs (not
     just `numCampaign`) to verify the disposal after deleting.
2. **Pre-flight checks:**
...
3. Only after they confirm, turn it off (via `edit-ai-group` / setting AI status to
