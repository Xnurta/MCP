...
- `get_ads_perf` → [query-ads-performance](../query-ads-performance/SKILL.md)
- `get_operation_log` → [query-operation-log](../query-operation-log/SKILL.md)
- `get_entity_metadata` → [query-entity-metadata](../query-entity-metadata/SKILL.md)

> **Dependency — install these three base skills alongside this one:** `query-ads-performance`, `query-entity-metadata`, `query-operation-log`. Skills install as sibling directories under `.claude/skills/<name>/`, which is exactly what the `../query-.../…` links above (and elsewhere in this doc) resolve against — if a base skill isn't installed, those links won't resolve and this skill can't run. Install all three first.
...
**Not for**:
- Monthly/quarterly view → `monthly-ads-report` / `quarterly-ads-report` (not yet built)
...
```
**Check `truncated`**: if `true`, this week had more than 200 matching AI/automation ops — follow `query-operation-log`'s "Getting a Complete Count" procedure (split into non-overlapping sub-windows and recurse) before summing; don't treat this one page as the full week.
...
**Must follow** [query-ads-performance's Period-over-Period and Top Movers example](../query-ads-performance/references/example-period-comparison-top-movers.md) in full — don't simplify by intuition. This is the section most prone to error and most likely to be challenged by a customer:
...
---
_Generated: {timestamp} · Data source: Xnurta MCP · Skill: weekly-ads-report v{version}_
```
...
- Deep root-cause attribution ("why did ACOS go up") → see `query-ads-performance`'s [ACOS root-cause investigation example](../query-ads-performance/references/example-acos-root-cause-investigation.md)
