...
- `get_operation_log` → [query-operation-log](../query-operation-log/SKILL.md) (used in Step 2e for the diagnostic card's correlated-changes check)
...
**Not for**:
- Account-wide spend/efficiency structure by campaign type / site / portfolio → `ads-structure-analysis`
- Periodic WoW/MoM cadence recaps → `weekly-ads-report` / `monthly-ads-report`
...
- **保 (Keep)**: Star or healthy Core tier — no action needed, or note it as a candidate for the *other* skill (`ads-structure-analysis`) if there's a structural budget-reallocation angle.
...
---
_生成时间：{timestamp} · 数据源：Xnurta MCP · Skill: product-diagnosis v{version}_
```
...
- Account-wide structural share analysis → `ads-structure-analysis`
- Periodic WoW/MoM cadence recaps → `weekly-ads-report` / `monthly-ads-report`
