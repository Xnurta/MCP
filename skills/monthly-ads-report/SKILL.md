...
This skill shares its overall shape with [weekly-ads-report](../weekly-ads-report/SKILL.md) (same underlying tools, same output-language and multi-store discipline) but goes deeper: AdGroup-level granularity, structural share analysis, product and keyword analysis, and a MoM+YoY baseline instead of WoW.
...
**Not for**:
- Weekly recap → `weekly-ads-report`
- Quarterly/QBR strategic review → `quarterly-ads-report` (not yet built)
- A specific question the user brings ("why did this one campaign's ACOS spike") → a dedicated analysis skill (not yet built), or the [ACOS root-cause investigation example](../query-ads-performance/references/example-acos-root-cause-investigation.md)
...
---
_Generated: {timestamp} · Data source: Xnurta MCP · Skill: monthly-ads-report v{version}_
```
...
- **Cannot reliably compute "real-time remaining budget"** — same caveat as weekly-ads-report; `dailyBudget`/`currentBudget` are configuration values, not a live spend ledger
