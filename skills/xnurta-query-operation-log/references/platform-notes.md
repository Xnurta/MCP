...
This file holds behavior that is **common to all 3 read tools** (`get_ads_perf`, `get_entity_metadata`, `get_operation_log`). It ships as `references/platform-notes.md` **inside this skill's own folder** — this copy travels with the `xnurta-query-operation-log` skill even if a Skill Hub installs/updates/downloads one skill folder at a time, since the reference file is nested under the same folder, not a sibling directory. The other two Xnurta skills (`xnurta-query-ads-performance`, `xnurta-query-entity-metadata`) each carry an identical copy of this file under their own `references/` folder for the same reason — this is intentional duplication for delivery robustness, not a mistake. Read this once per session before making tool-selection decisions.
